...
using MizukiShigeruVPS.ContentManagement;
        ...
        private IEnumerator InvokeCallback(string json)
        {
            ...
            if (browserFiles.Count > 0)
            {
                for (var i = 0; i < browserFiles.Count; i++)
                {
                    ...
                    var url = browserFile.SelectToken("url").ToString();
                    var loader = new WWW(url);

                    var uploader = FindObjectOfType<ObjectListView>();
                    uploader.BrowserFilePath = url;
